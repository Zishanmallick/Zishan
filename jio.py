...
import os
import json # Import json to parse the credentials string
...
# Removed GOOGLE_CREDS_FILE as we will now use an environment variable for credentials.
...
def init_sheets_client():
    ...
    It attempts to load service account credentials from an environment variable
    named 'GOOGLE_SHEETS_CREDENTIALS'.
    This is a more secure way to handle credentials in deployment environments.
    """
    try:
        ...
        if creds_json_str:
            try:
                ...
                st.error(f"Error decoding GOOGLE_SHEETS_CREDENTIALS environment variable. Ensure it's valid JSON: {e}")
                st.stop()
            except Exception as e:
                st.error(f"Error authenticating with GOOGLE_SHEETS_CREDENTIALS environment variable: {e}")
                st.stop()
        else:
            st.error("GOOGLE_SHEETS_CREDENTIALS environment variable not found. Please set this environment variable with your service account JSON key content.")
            st.stop() # Stop the app if credentials cannot be found

    except Exception as e:
        st.error(f"Failed to initialize Google Sheets client. Please ensure credentials are correct and have necessary permissions: {e}")
...
def display_blog_board():
    ...
    if 'temp_blog_title' not in st.session_state: # Initialize for new post form
        st.session_state.temp_blog_title = ""
    if 'temp_blog_content' not in st.session_state: # Initialize for new post form
...
def logout():
    ...
    if st.sidebar.button("Logout"):
        ...
        st.session_state.edit_blog_title = ""
        st.session_state.edit_blog_content = ""
        # Clear tracker related session states
        st.session_state.editing_tracker_issue_id = None
        st.session_state.editing_tracker_data = {}
        st.rerun()
...
if 'logged_in' not in st.session_state:
    ...
    st.session_state.user_name = ""
    st.session_state.is_admin = False
    # Initialize blog post related session states for the first run
    st.session_state.temp_blog_title = ""
    st.session_state.temp_blog_content = ""
    st.session_state.edit_blog_idx = None
    st.session_state.edit_blog_title = ""
    st.session_state.edit_blog_content = ""
    # Initialize tracker related session states for the first run
    st.session_state.editing_tracker_issue_id = None
    st.session_state.editing_tracker_data = {}
...
if not st.session_state.logged_in:
    ...
    tab_names = ["Weekly Tasks", "Blog Board", "Intern Profiles"]
    if st.session_state.is_admin or st.session_state.user_name == "Policy":
        tab_names.append("Issue Tracker") # Only show tracker to Admin and Policy

    selected_tab = st.sidebar.radio("Navigation", tab_names)

    if selected_tab == "Weekly Tasks":
        display_tasks()
        ...
        if st.session_state.is_admin or st.session_state.user_name == "Policy":
            ...
            st.error("Access Denied: You do not have permission to view the Issue Tracker.")
